...
class viewer_command_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(viewer_command_t._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != viewer_command_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return viewer_command_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = viewer_command_t()
    ...
    def _get_hash_recursive(parents):
        if viewer_command_t in parents: return 0
    ...
    def _get_packed_fingerprint():
        if viewer_command_t._packed_fingerprint is None:
            viewer_command_t._packed_fingerprint = struct.pack(">Q", viewer_command_t._get_hash_recursive([]))
        return viewer_command_t._packed_fingerprint
