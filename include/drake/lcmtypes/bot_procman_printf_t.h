...
#ifndef _bot_procman_printf_t_h
#define _bot_procman_printf_t_h
 ...
 *
 */
typedef struct _bot_procman_printf_t bot_procman_printf_t;
struct _bot_procman_printf_t
{
    int64_t    utime;
    char*      deputy_name;
    int32_t    sheriff_id;
...
/**
 * Create a deep copy of a bot_procman_printf_t.
 * When no longer needed, destroy it with bot_procman_printf_t_destroy()
 */
bot_procman_printf_t* bot_procman_printf_t_copy(const bot_procman_printf_t* to_copy);

/**
 * Destroy an instance of bot_procman_printf_t created by bot_procman_printf_t_copy()
 */
void bot_procman_printf_t_destroy(bot_procman_printf_t* to_destroy);

/**
 * Identifies a single subscription.  This is an opaque data type.
 */
typedef struct _bot_procman_printf_t_subscription_t bot_procman_printf_t_subscription_t;

/**
 * Prototype for a callback function invoked when a message of type
 * bot_procman_printf_t is received.
 */
typedef void(*bot_procman_printf_t_handler_t)(const lcm_recv_buf_t *rbuf,
             const char *channel, const bot_procman_printf_t *msg, void *userdata);

/**
 * Publish a message of type bot_procman_printf_t using LCM.
 *
 ...
 */
int bot_procman_printf_t_publish(lcm_t *lcm, const char *channel, const bot_procman_printf_t *msg);

/**
 * Subscribe to messages of type bot_procman_printf_t using LCM.
 *
 ...
 */
bot_procman_printf_t_subscription_t* bot_procman_printf_t_subscribe(lcm_t *lcm, const char *channel, bot_procman_printf_t_handler_t handler, void *userdata);

/**
 * Removes and destroys a subscription created by bot_procman_printf_t_subscribe()
 */
...
/**
 * Encode a message of type bot_procman_printf_t into binary form.
 *
 * @param buf The output buffer.
 * @param offset Encoding starts at this byte offset into @p buf.
 * @param maxlen Maximum number of bytes to write.  This should generally
 *               be equal to bot_procman_printf_t_encoded_size().
...
/**
 * Decode a message of type bot_procman_printf_t from binary form.
 * When decoding messages containing strings or variable-length arrays, this
 * function may allocate memory.  When finished with the decoded message,
 * release allocated resources with bot_procman_printf_t_decode_cleanup().
 *
 ...
 */
int bot_procman_printf_t_decode(const void *buf, int offset, int maxlen, bot_procman_printf_t *msg);

/**
 * Release resources allocated by bot_procman_printf_t_decode()
 * @return 0
 */
int bot_procman_printf_t_decode_cleanup(bot_procman_printf_t *p);

/**
 * Check how many bytes are required to encode a message of type bot_procman_printf_t
 */
