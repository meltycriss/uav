...
int lcmt_driving_command_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->brake, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int lcmt_driving_command_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->brake, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    uint64_t hash = 0x3e027f0e6c8de5abLL;
